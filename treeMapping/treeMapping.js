...
Tree.prototype.map = function(callback, currentNode) {

  currentNode = currentNode || new Tree(callback(this.value));
  for (var i = 0 ; i < this.children.length ; i++) {
    currentNode.addChild(callback(this.children[i].value));
    if(this.children[i].children.length > 0) {
      this.children[i].map(callback, currentNode.children[i]);
    }
  };
  return currentNode; 
}
